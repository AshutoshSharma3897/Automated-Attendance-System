...
window = tk.Tk()
window.title("Automated Attendance System using Face Recognition")
...
def manually_fill():
    ...
    def fill_attendance():
        ...
        try:
            global cursor
            connection = pymysql.connect(host='localhost', user='root', password='', db='attendance_system')
        ...
        if subb=='':
            ...
            def err_screen1():
                ...
                Label(errsc2, text='Please enter Student Name & Enrollment No!!!', fg='white', bg='black',
            ...
            ENR = tk.Label(MFW, text="Enter Enrollment", width=15, height=2, fg="black", bg="cyan",
                           font=('times', 15, ' bold '))
            ENR.place(x=30, y=100)

            STU_NAME = tk.Label(MFW, text="Enter Student name", width=15, height=2, fg="black", bg="cyan",
                                font=('times', 15, ' bold '))
            ...
            def create_csv():
                ...
                csv_name='G:/Ml/Minor/Attendance/Manually Attendance/'+DB_table_name+'.csv'
                with open(csv_name, "w") as csv_file:
                    csv_writer = csv.writer(csv_file)
                    csv_writer.writerow([i[0] for i in cursor.description])  # write headers
                    csv_writer.writerows(cursor)
                    O="CSV created Successfully"
                    Notifi.configure(text=O, bg="white", fg="black", width=33, font=('times', 19, 'bold'))
            ...
            Notifi = tk.Label(MFW, text="CSV created Successfully", bg="cyan", fg="black", width=33,
            ...
            DATA_SUB = tk.Button(MFW, text="Enter Data",command=enter_data_DB, fg="black", bg="cyan", width=20,
                                 height=2,
                                 activebackground="Red", font=('times', 15, ' bold '))
            ...
            def attf():
                import subprocess
                subprocess.Popen(r'explorer /select,"G:/Ml/Minor/Attendance\Manually Attendance"')
    ...
    SUB = tk.Label(sb, text="Enter Subject", width=15, height=2, fg="black", bg="cyan", font=('times', 15, ' bold '))
    ...
    fill_manual_attendance = tk.Button(sb, text="Fill Attendance",command=fill_attendance, fg="black", bg="cyan", width=20, height=2,
                       activebackground="Red", font=('times', 15, ' bold '))
...
def err_screen():
    ...
    Label(sc1,text='Enrollment No & Student Name required!!!',fg='white',bg='black',font=('times', 16, ' bold ')).pack()
    Button(sc1,text='OK',command=del_sc1,fg="black"  ,bg="white"  ,width=9  ,height=1, activebackground = "Red" ,font=('times', 15, ' bold ')).place(x=90,y= 50)
...
def err_screen1():
    ...
    Button(sc2,text='OK',command=del_sc2,fg="black"  ,bg="white"  ,width=9  ,height=1, activebackground = "Red" ,font=('times', 15, ' bold ')).place(x=90,y= 50)
...
def subjectchoose():
    ...
    Notifica = tk.Label(windo, text="Attendance filled Successfully", bg="black", fg="white", width=33,
                            height=2, font=('times', 15, 'bold'))

    def Attf():
        import subprocess
        subprocess.Popen(r'explorer /select,"G:/Ml/Minor\Attendance\"')

    attf = tk.Button(windo,  text="Check Sheets",command=Attf,fg="black"  ,bg="cyan"  ,width=12  ,height=1 ,activebackground = "Red" ,font=('times', 14, ' bold '))
    attf.place(x=430, y=255)


    sub = tk.Label(windo, text="Enter Subject", width=15, height=2, fg="black", bg="cyan", font=('times', 15, ' bold '))
    sub.place(x=30, y=100)

    tx = tk.Entry(windo, width=20, bg="white", fg="black", font=('times', 23, ' bold '))
    tx.place(x=250, y=105)

    fill_a = tk.Button(windo, text="Fill Attendance", fg="black",command=Fillattendances, bg="cyan", width=20, height=2,
                       activebackground="Red", font=('times', 15, ' bold '))
...
def admin_panel():
    ...
    def log_in():
        ...
        if username == 'aaryandas99' :
            if password == 'AryanDas':
                win.destroy()
                import csv
                import tkinter
                root = tkinter.Tk()
                ...
                cs = 'G:/Ml/Minor/StudentDetails/StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
                ...
                Nt.configure(text=valid, bg="cyan", fg="black", width=38, font=('times', 19, 'bold'))
    ...
    un = tk.Label(win, text="Enter username", width=15, height=2, fg="black", bg="cyan",
                   font=('times', 15, ' bold '))
    un.place(x=30, y=50)

    pw = tk.Label(win, text="Enter password", width=15, height=2, fg="black", bg="cyan",
                  font=('times', 15, ' bold '))
    pw.place(x=30, y=150)
    ...
    Login = tk.Button(win, text="Login", fg="black", bg="cyan", width=20,
                       height=2,
...
message = tk.Label(window, text="Automated-Attendance-System-using-Face-Recognition", bg="cyan", fg="black", width=50,
...
txt = tk.Entry(window, validate="key", width=20, bg="white", fg="red", font=('times', 25, ' bold '))
...
txt2 = tk.Entry(window, width=20, bg="white", fg="red", font=('times', 25, ' bold '))
...
AP = tk.Button(window, text="Check Registered Students",command=admin_panel,fg="white"  ,bg="red"  ,width=19 ,height=1, activebackground = "Red" ,font=('times', 15, ' bold '))
AP.place(x=990, y=410)

takeImg = tk.Button(window, text="Take Images",command=take_img,fg="black"  ,bg="cyan"  ,width=20  ,height=3, activebackground = "Red" ,font=('times', 15, ' bold '))
takeImg.place(x=90, y=500)

trainImg = tk.Button(window, text="Train Model",fg="black",command=training ,bg="cyan"  ,width=20  ,height=3, activebackground = "Red" ,font=('times', 15, ' bold '))
trainImg.place(x=390, y=500)

FA = tk.Button(window, text="Automatic Attendance",fg="black",command=subjectchoose  ,bg="cyan"  ,width=20  ,height=3, activebackground = "Red" ,font=('times', 15, ' bold '))
FA.place(x=690, y=500)

quitWindow = tk.Button(window, text="Manually Fill Attendance", command=manually_fill  ,fg="black"  ,bg="cyan"  ,width=20  ,height=3, activebackground = "Red" ,font=('times', 15, ' bold '))
